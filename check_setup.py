...
def test_config_and_keys():
    ...
    # Check character.yaml
    print_info("Checking character.yaml...")
    config_path = 'character.yaml'
    if not os.path.exists(config_path):
        print_error(f"{config_path} not found")
        issues.append("character.yaml missing")
    else:
        ...
        try:
            ...
            required_fields = ['presets']
            for field in required_fields:
                if field in config:
                    print_success(f"  - {field}: present")
                else:
                    print_error(f"  - {field}: missing")
                    issues.append(f"character.yaml missing '{field}'")
            ...
            issues.append(f"character.yaml parse error: {e}")
    ...
    if issues:
        ...
        if "OPENAI_API_KEY not set" in str(issues):
            ...
            print("  2. Add: OPENROUTER_API_KEY=sk-your api key")
...
# ============================================================================
def test_openrouter():
    print_header("TEST 1: LLM (OpenRouter) Connection")

    print_info("Testing connection to OpenRouter API...")
    ...
    try:
        ...
        if response.status_code != 200:
            print_error(f"OpenRouter request failed: {response.status_code}")
            print_info(str(response.text))
            return False

        data = response.json()

        # Jeśli API zwróciło choices → model działa
        if "choices" in data and len(data["choices"]) > 0:
            print_success("Model responded.")
            print_success("OpenRouter API is working correctly!")
            return True
        else:
            print_error("Model did not return choices → API reachable but model failed.")
            return False
...
def test_audio_recording():
    ...
    try:
        # Twoja funkcja nagrywania
        from funcs.stt.stt_record import record_on_speech

        test_audio_path = Path("data/audio") / "test_recording.wav"
        ...
        # Sprawdzenie długości nagrania
        import soundfile as sf
        with sf.SoundFile(test_audio_path) as f:
            duration = len(f) / f.samplerate

        print_info(f"Recorded duration: {duration:.2f} seconds")

        if duration < 0.4:
            print_error("Recording too short — speak a bit longer.")
            return False
        ...
        try:
            ...
            transcription = transcribe_audio_groq(test_audio_path)
            ...
            print_warning("Check GROQ_API_KEY and internet connection.")
            return False

    except Exception as e:
        print_error(f"Recording test failed: {e}")
        print_warning("Check microphone permissions and audio settings.")
        return False


# ============================================================================
# TEST 3: Translation Test (GoogleTranslator)
# ============================================================================
def test_translation():
    print_header("TEST 7: Translation Test (GoogleTranslator)")

    try:
        from deep_translator import GoogleTranslator
    except ImportError:
        print_error("deep_translator is not installed.")
        print_warning("Install with: pip install deep-translator")
        return False

    # 🔥 TU WPISUJESZ TEKST DO TŁUMACZENIA
    text = "To jest przykładowy tekst do przetłumaczenia."

    if not text.strip():
        print_error("No text provided for translation.")
        return False

    print_info(f"Text to translate: {text[:60]}...")
    ...
    # Zapis wyniku (opcjonalnie)
    output_path = Path("data/global_data/translate_answer.json")
    try:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump({"text": translated}, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print_error(f"Failed to save translation: {e}")
        return False

    print_success(f"Translation successful! Saved to: {output_path}")
    print_info(f"Translated text: {translated}")
...
# ============================================================================
# TEST 4: TTS (SoVITS) Audio Generation
# ============================================================================
def test_tts_generation():
    ...
    try:
        ...
        # Katalog projektu — ZAWSZE lokalny
        audio_dir = Path("data/audio")
        audio_dir.mkdir(parents=True, exist_ok=True)

        # Usuń stare testowe nagrania
        for f in audio_dir.glob("tts_test_*.wav"):
            try:
                f.unlink()
                print_info(f"🗑 Deleted old test audio: {f.name}")
            except Exception as e:
                print_warning(f"Could not delete {f.name}: {e}")

        # Tekst testowy
        raw_text = "Hello! This is a test of the text to speech system."
        clean_text = clean_llm_output(raw_text)

        # Ścieżka wyjściowa — zawsze w data/audio
        output_path = audio_dir / f"tts_test_{uuid.uuid4().hex}.wav"

        print_info(f"Generating speech for: '{clean_text}'")
        print_warning("This requires GPT-SoVITS server to be running...")

        # Generowanie audio
        result_path = sovits_gen(clean_text, str(output_path))

        if result_path is None:
            print_error("TTS generation failed — sovits_gen returned None")
            return False
        ...
        if output_path.exists() and output_path.stat().st_size > 1000:
            ...
            try:
                ...
                print_warning("Could not read WAV duration")

            print_success("✓ TTS generation is working!")
            return True
        ...
        print_warning("Check if GPT-SoVITS server is running and configured correctly.")
        return False


# ============================================================================
# TEST 5: VRM Server and Audio Playback
# ============================================================================
...
def test_vrm_audio_playback():
    ...
    audio_dir = Path("data/audio")  # serwer serwuje stąd
    audio_dir.mkdir(parents=True, exist_ok=True)
    for f in audio_dir.glob("test_playback_*.wav"):
        try:
            f.unlink()
            print_info(f"Deleted old audio: {f}")
        except Exception as e:
            print_warning(f"Could not delete {f}: {e}")

    try:
        from funcs.tts.sovits_ping import sovits_gen, get_wav_duration

        # Generuj bezpośrednio do data/audio/
        filename = f"test_playback_{uuid.uuid4().hex}.wav"
        audio_path = audio_dir / filename

        test_text = "Testing audio playback through the VRM system."

        print_info("Generating test audio...")
        sovits_gen(test_text, str(audio_path))

        duration = get_wav_duration(str(audio_path))
        print_success(f"Test audio generated: {audio_path}")

        # URL do serwera (serwer serwuje /audio/nazwa.wav)
        audio_url = f"http://127.0.0.1:8001/audio/{filename}"

        print_info("Sending audio to VRM server...")
        url = f"{BASE_URL}/talk"
        payload = {
            "audio_path": audio_url,           # pełny URL
            "expression": "relaxed",
        ...
        if response.status_code == 200:
            print_success("VRM talk request successful!")
