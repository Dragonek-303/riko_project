...
// --- SUBTITLE QUEUE SYSTEM (STABLE VERSION) ---
let subtitleQueue = Promise.resolve();

function queueSubtitle(text, audioDuration) {

    // If no text or duration is 0 → it's another audio chunk, ignore
    if (!text || !audioDuration || audioDuration <= 0) {
        return;
    }

    subtitleQueue = subtitleQueue.then(() => {
        return new Promise(resolve => {
            const sub = document.getElementById("subtitle-container");
            if (!sub) return resolve();

            // One full block of text
            sub.innerText = text;
            sub.classList.add("visible");

            
            const hold = audioDuration * 1000;
...
(async () => {
 ...
 // renderer.setClearColor(0x000000, 0); // black color, alpha = 0
//////////////////////////////////////////////////////////////////////////
// load a room 
  ...
  //   model.scale.set(0.35, 0.35, 0.35); // Start with no scale reduction
  //   model.position.set(-1.5, 0.33, -0.7); // x,y,z
...
/////////////////////////////////////////////////////////////////////////////////
  ...
  ws.onmessage = async ({ data }) => {
    ...
    // BLOCKADE: if a dance (VRMA) is in progress, ignore new animations/speech
    if (animationMgr && (animationMgr.isVRMAPlaying())) {
        const blockingTypes = ['start_animation', 'start_vrma', 'start_mixamo', 'set_state', 'talk'];
        if (blockingTypes.includes(msg.type)) {
            console.warn('🚫 Zablokowano komendę', msg.type, '– trwa taniec');
            return;
        }
    }
    ...
    if (msg.type === 'set_state') {
      ...
      if (animationMgr && ['idle', 'listening', 'thinking', 'talking'].includes(state)) {
        ...
        // CLEAR SUBTITLES WHEN TRANSITIONING TO IDLE
        if (state === 'idle') {
          //const sub = document.getElementById("subtitle-container");
          //sub.innerText = "";
          //sub.classList.remove("visible");
    ...
    if (msg.type === 'start_animation') {
      ...
      console.log("RECEIVED TEXT:", audio_text);

      // QUEUE SUBTITLES
      queueSubtitle(audio_text, audio_duraction);

      // AUDIO + ANIMATION
      audioMgr.setExpression(expression);

      // Build full URL – if already absolute, use it; otherwise add base
      let fullAudioUrl;
      if (audio_path.startsWith('http://') || audio_path.startsWith('https://')) {
          fullAudioUrl = audio_path;
      } else if (audio_path.startsWith('/')) {
          fullAudioUrl = `http://localhost:8001${audio_path}`;
      } else {
          fullAudioUrl = `http://localhost:8001/${audio_path}`;
      }
      ...
      try {
          ...
          const ok = await playbackController.playAudioUrl(fullAudioUrl);
    ...
    if (msg.type === 'change_skin') {
      const { skin_path } = msg;
      console.log('👗 Changing skin to:', skin_path);

      try {
        const newVrmData = await loadVRM(skin_path, scene);
        const newVrm = newVrmData.vrm;
        ...
        if (audioMgr) audioMgr.vrm = vrm;
        if (animationMgr) animationMgr.vrm = vrm;
        animationMgr.setState('idle');
  ...
  // ===============================
  // MANUAL TEXT INPUT → send to backend
  // ===============================
  ...
  fileInput.addEventListener("change", async () => {
      ...
      try {
          const res = await fetch("http://127.0.0.1:8001/upload_image", {
              method: "POST",
              body: formData,
          });
